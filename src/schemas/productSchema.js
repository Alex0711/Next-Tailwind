...
const title = Joi.string().min(3).max(25).label("title"); //no acepta espacios por alphanum
const description = Joi.string().min(10).max(255).label("description");
const price = Joi.number().integer().min(10);
const category = Joi.number().valid(1, 2, 3, 4, 5);
const images = Joi.array(); //uri creo que valida si es la url de una imagen
